...
class Tab1(QWidget):
    ...
    def update_blockchain(self):
        ...
        for block in self.devs.chain:
            if block['transaction']['type'] == 'open':
                ...
                self.vote_list[block['transaction']['data']['id']]['vote_count'][
                    block['transaction']['data']['vote']] += 1
            elif block['transaction']['type'] == 'mine':
                if block['transaction']['data']['address'] == self.devs.wallet_address:
                    self.devs.balance += block['transaction']['data']['reward']
            elif block['transaction']['type'] == 'deposit':
                if block['transaction']['data']['from_address'] == self.devs.wallet_address:
                    self.devs.balance -= block['transaction']['data']['amount']
                if block['transaction']['data']['to_address'] == self.devs.wallet_address:
                    self.devs.balance += block['transaction']['data']['amount']
    ...
    def update_vote(self):
        ...
        self.option_progress1.setValue(
            self.vote_list[self.current_vote_id]['vote_count'][self.vote_list[self.current_vote_id]['options'][0]])
        ...
        self.option_progress2.setValue(
            self.vote_list[self.current_vote_id]['vote_count'][self.vote_list[self.current_vote_id]['options'][1]])
        ...
        self.option_progress3.setValue(
            self.vote_list[self.current_vote_id]['vote_count'][self.vote_list[self.current_vote_id]['options'][2]])
...
class Tab3(QWidget):
    ...
    def mine(self):
        unconfirmed_block_count = len(self.devs.unconfirmed_blocks)
        for unconfirmed_block in self.devs.unconfirmed_blocks:
        ...
        self.unconfirmed_block_list_widget.clear()
        self.devs.vote_list_tab.update_blockchain()
        unconfirmed_block = {
            'transaction': {
                'author': self.devs.public_key.to_pem().decode(),
                'type': 'mine',
                'data': {
                    'address': self.devs.wallet_address,
                    'reward': unconfirmed_block_count
                }
            }
        }
        unconfirmed_block['signature'] = get_block_signature(unconfirmed_block, self.devs.private_key)
...
class Tab4(QWidget):
    def __init__(self, devs):
        ...
        deposit_clear_layout = QHBoxLayout()
        deposit_button = QPushButton("송금")
        deposit_button.clicked.connect(self.deposit)
        clear_button = QPushButton("초기화")
        clear_button.clicked.connect(self.clear_form)
        deposit_clear_layout.addWidget(deposit_button)
        deposit_clear_layout.addWidget(clear_button)

        form_layout.addRow("지갑 주소: ", self.address_line_edit)
        form_layout.addRow("금액: ", self.amount_line_edit)
        form_layout.addRow("", deposit_clear_layout)
    ...
    def deposit(self):
        unconfirmed_block = {
            'transaction': {
                ...
                'type': 'deposit',
                'data': {
                    'from_address': self.devs.wallet_address,
                    'to_address': self.address_line_edit.text(),
                    'amount': int(self.amount_line_edit.text()),
                }
            }
        }
        unconfirmed_block['signature'] = get_block_signature(unconfirmed_block, self.devs.private_key)
...
class SocketReceiver(QThread):
    update_blockchain_signal = pyqtSignal()
        ...
        while True:
            ...
            if data['transaction']['type'] == 'connect':
                ...
                if verify_block_chain(data['transaction']['data']['chain']):
                    ...
                    self.update_blockchain_signal.emit()
                else:
                    print('블록체인 위변조 감지')
                ...
                self.devs.unconfirmed_blocks.append(data)
                self.update_unconfirmed_block_list_signal.emit()
            elif data['transaction']['type'] == 'deposit':
                self.devs.unconfirmed_blocks.append(data)
                self.update_unconfirmed_block_list_signal.emit()
...
class SocketListener(QThread):
    ...
    def run(self):
        while True:
            ...
            self.receive_thread.update_blockchain_signal.connect(self.update_blockchain)
...
class SocketManager(QThread):
    ...
    def run(self):
        ...
        self.listen_thread.update_blockchain_signal.connect(self.update_blockchain)
...
class DecentralizedElectronicVotingSystem(QWidget):
    def __init__(self):
        ...
        genesis_block['signature'] = get_block_signature(genesis_block, self.private_key)
        genesis_block['hash'] = get_block_hash(genesis_block)

        genesis_reward_block = {
            'transaction': {
                'author': self.public_key.to_pem().decode(),
                'type': 'mine',
                'data': {
        ...
        genesis_reward_block['signature'] = get_block_signature(genesis_reward_block, self.private_key)
        genesis_reward_block['hash'] = get_block_hash(genesis_reward_block)

        self.chain = [genesis_block, genesis_reward_block]
    ...
    @pyqtSlot()
    def update_blockchain(self):
        self.vote_list_tab.update_blockchain()
